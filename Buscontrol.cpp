...
Buscontrol::Buscontrol(string nombre)
{
  registroAcumulador=0;
}
...
{ 
  ...
  bool present = false;
  string  key = nombre;
  int size=sizeof(nombreMemoria)/sizeof(nombreMemoria[0]);
...
void Buscontrol:: unidadcontrol(string instruccion,string DireccionMemoria,string  numero,string entrada,string entrada2) 
{
  ...
  */
	...
	if(instruccion=="LDR") 
  {
		...
		dirrecion (DireccionMemoria);
		int  memoria2 = conteo;
		registroAcumulador=memoriaA[memoria2];  
	} 
  ...
  if(instruccion=="ADD") 
  {
    if( numero == "NULL")
    {
      dirrecion (DireccionMemoria);
		  ...
		  int copia= registroAcumulador;
		  int sum = memoria+copia;
		  registroAcumulador= sum;
      //cout<<"RESGISTRO SUMA :" <<registroAcumulador<<endl;
  ...
  if(instruccion=="STR") 
  {
	  ...
	  int numeroAcumalado= registroAcumulador;
...
*/
  ...
  if(instruccion=="SUB") 
  {
    if( numero == "NULL")
    {
      dirrecion (DireccionMemoria);
		  ...
		  registroAcumulador= rest;
		  //cout << " RESTA " << registroAcumulador << endl;
  ...
  if(instruccion=="MUL") 
  {
    if( numero == "NULL")
    {
      ...
      for(int j=0; j<copia;j++ )
      {
       ...
       //cout<<"RESGISTRO :" <<registroAcumulador;
    ...
    //cout<<"FINAL : " << registroAcumulador<<endl;
      
    }else if (entrada!="NULL")
    {
      int copiaRegistroAcumulador = registroAcumulador;
       dirrecion (DireccionMemoria);
		    int  memoria = memoriaA[conteo];
        dirrecion (numero);
        int  memoria2 = memoriaA[conteo];
      ...
      for(int j=0; j< memoria2-1;j++ )
      {
        ...
        cout<<"FINAL : " << registroAcumulador<<endl;
         ...
         dirrecion(entrada);
         memoriaA[conteo]=registroAcumulador;

      registroAcumulador = copiaRegistroAcumulador;
  ...
  if(instruccion=="DIV") 
  {
    ...
    if( numero == "NULL")
    {
      ...
      // 5/ 3 =  1
      int cociente = 0;
      while ( registroAcumulador > memoria)
        {
          unidadcontrol("SUB",DireccionMemoria,"NULL","NULL","NULL");
          cociente ++;

      }
      registroAcumulador= cociente;
      //cout << "DIV "<<registroAcumulador<<endl;
    }
    ...
    else if(numero!="NULL"&& entrada=="NULL")
    {
      ...
      //divisor = memoria2
      int cociente = 0;
      while(registroAcumulador > memoria2 )
      {
